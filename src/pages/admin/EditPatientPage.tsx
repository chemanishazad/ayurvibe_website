...
import { ArrowLeft, Loader2, User, MapPin, Stethoscope, UserPen, AlertCircle } from 'lucide-react';
import {
  Select,
  SelectContent,
  SelectItem,
...
const EditPatientPage = () => {
  ...
  const setAgeUnit = (u: AgeUnit) => {
    const digits = String(watch('age') ?? '').replace(/\D/g, '');
  ...
  useEffect(() => {
    if (!id) { navigate('/admin/patients'); return; }
    api.patients.get(id)
      .then((p) => {
    ...
    <div className="flex w-full min-w-0 flex-col flex-1 min-h-0 gap-4">
      <div className="flex w-full min-w-0 shrink-0 items-stretch gap-3 rounded-xl border border-border/60 bg-card/95 p-3 shadow-sm ring-1 ring-black/[0.04] dark:ring-white/[0.06] sm:items-center sm:p-4">
        <Button variant="ghost" size="icon" className="h-10 w-10 shrink-0 rounded-lg border border-transparent hover:border-border hover:bg-muted/60" asChild>
          <Link to="/admin/patients" aria-label="Back to patients"><ArrowLeft className="h-4 w-4" /></Link>
        </Button>
        ...
        <CardContent className="w-full min-w-0 space-y-6 p-4 sm:p-6">
          <form onSubmit={handleSubmit(onSubmit)} noValidate>
            <div className="space-y-6">
              {/* Patient Profile */}
              <PatientFormSection icon={<User className="h-5 w-5" />} title="Patient profile" description="Legal name, contact, age, and gender.">
                <div className="w-full space-y-5">
                  ...
                  <div className="w-full min-w-0">
                    <Label className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Mobile <span className="text-destructive">*</span></Label>
                    <div className="mt-1.5 flex flex-col gap-2 sm:flex-row">
                      <Select value={watch('countryCode') || '91'} onValueChange={(v) => setValue('countryCode', v)}>
                        <SelectTrigger className="h-10 w-full sm:w-[140px] sm:shrink-0"><SelectValue placeholder="Code" /></SelectTrigger>
                        <SelectContent>
                          {COUNTRY_CODES.map((c) => (
                            <SelectItem key={c.code} value={c.code}>{c.dial} {c.country}</SelectItem>
                          ))}
                        </SelectContent>
                      </Select>
                      <Input
                        placeholder="10-digit number" maxLength={10} inputMode="numeric"
                        className={cn('h-10 min-w-0 flex-1', errors.mobile && 'border-destructive')}
                        {...register('mobile', { onChange: (e) => { e.target.value = e.target.value.replace(/\D/g, '').slice(0, 10); } })}
                      />
                    </div>
                  ...
                  <div className="w-full min-w-0">
                    <Label className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Gender <span className="text-destructive">*</span></Label>
                    <div className={cn('mt-2 grid grid-cols-3 gap-2', errors.gender && 'rounded-lg p-0.5 ring-2 ring-destructive/25')} role="radiogroup" aria-label="Gender">
                      {GENDERS.map((g) => {
                        const selected = gender === g;
                        return (
                          <button key={g} type="button" role="radio" aria-checked={selected}
                            onClick={() => setValue('gender', g, { shouldValidate: true })}
                            className={cn('rounded-lg border-2 px-2 py-2.5 text-center text-xs font-semibold transition-all duration-150 sm:px-3 sm:py-3 sm:text-sm', selected ? choiceActive : choiceIdle)}>
                            {g}
                          </button>
                        );
                      })}
                  ...
                  <div className="w-full min-w-0">
                    <Label className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Age <span className="text-destructive">*</span></Label>
                    <p className="mt-1 text-xs text-muted-foreground">Choose years or months, then enter a whole number.</p>
                    <div className={cn('mt-2 grid w-full grid-cols-2 gap-2', errors.age && 'rounded-lg p-0.5 ring-2 ring-destructive/25')} role="radiogroup" aria-label="Age unit">
                      {(['years', 'months'] as const).map((unit) => (
                        <button key={unit} type="button" role="radio" aria-checked={ageUnit === unit} onClick={() => setAgeUnit(unit)}
                          className={cn('rounded-lg border-2 px-4 py-3 text-sm font-semibold transition-all duration-150', ageUnit === unit ? choiceActive : choiceIdle)}>
                          {unit.charAt(0).toUpperCase() + unit.slice(1)}
                        </button>
                      ))}
                    </div>
                    <Input
                      type="text" inputMode="numeric" value={watch('age') ?? ''} onChange={(e) => handleAgeInput(e.target.value)}
              ...
              {/* Address */}
              <PatientFormSection icon={<MapPin className="h-5 w-5" />} title="Address" description="Enter the full address first, then pincode if you have it.">
                <div className="space-y-4">
                  <div>
                    <Label className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Full address <span className="text-destructive">*</span></Label>
                    <Textarea placeholder="Street, area, city, state" rows={4} className={cn('mt-1.5 min-h-[120px] resize-y bg-background/80', errors.address && 'border-destructive')} {...register('address')} />
                    <FieldError message={errors.address?.message} />
                  </div>
                  <div>
                    <Label className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Pincode (India)</Label>
                    <Input placeholder="6-digit pincode (optional)" maxLength={6} inputMode="numeric" className="mt-1.5 h-11 w-full"
                      {...register('pincode', { onChange: (e) => { e.target.value = e.target.value.replace(/\D/g, '').slice(0, 6); } })} />
              ...
              {/* Medical History */}
              <PatientFormSection icon={<Stethoscope className="h-5 w-5" />} title="Medical history" description="Optional. Allergies, chronic conditions, surgeries.">
                <div>
                  <Label className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Notes <span className="text-muted-foreground font-normal normal-case">(optional)</span></Label>
                  <Textarea placeholder="Past conditions, allergies, ongoing medication, etc." rows={4} className="mt-1.5 min-h-[100px] resize-y bg-background/80" {...register('medicalHistory')} />
                </div>
              </PatientFormSection>
